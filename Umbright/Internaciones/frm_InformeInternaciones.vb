...
Public Class frm_informeInternaciones
    Dim Ods As DataSet
    ...
    Private Sub llenarCombos()
        ...
        Try
            ...
            clsgen.fillComboBox(Otrans, lsSQL, "tipodocumento", "tipoDocto", "tipoDocto", cmbTipoDocto)


            lsSQL = "pa_sel_um_gen_tabcod null,'gen_bodega','" & gs_empresa & "'"
            clsgen.fillComboBox(Otrans, lsSQL, "bodega", "CODIGO", "CODIGO", cmbBodega)

            lsSQL = "pa_sel_um_gen_tabcod null,'gen_motivos_bodega','" & gs_empresa & "'" 'Solo en DM se Almacenan los motivos
            clsgen.fillComboBox(Otrans, lsSQL, "motivo", "DESCRIPCION", "CODIGO", cmbMotivoInternacion)
    ...
    Private Sub buscarDocumento()
        ...
        Try
            lsSQL = "pa_sel_um_documento_detalle_proveedor '" & Me.cmbTipoDocto.SelectedValue & "','" & gs_empresa & "','" & Me.txtNumero.Text & "'"
            ...
            If dt.Rows.Count > 0 Then
                ...
                clsGen.Alinear_GridView(Ods.Tables("documento"), dgvDetalleDocumento, ",producto,glosa,cantidad,precio,lote,fechavctod,", "", "", "", "", "", ",producto,glosa,cantidad,precio,", True, True, 250, 0)
                If dt.Rows.Count > 0 Then
                    Me.txtBodega.Text = dt.Rows(0).Item("Bodega")
                    Me.txtProveedor.Text = dt.Rows(0).Item("proveedor")
                    Me.txtFecha.Text = dt.Rows(0).Item("fecha")
                    Me.txtComentario4.Text = dt.Rows(0).Item("comentario4").ToString
                End If
            Else
    ...
    Private Function realizarSalida(ByRef psTipoDocto As String, ByRef psNumeroDocto As String, ByRef piCorrelativo As Integer) As Integer
        ...
        dr.Item("Glosa") = "Referencia " & Me.cmbTipoDocto.SelectedValue & "-" & Me.txtNumero.Text  'drEncabezado.Item("TipoDoctoFactura") & "-" & drEncabezado.Item("NumeroFactura") 'Validar Glosa
        ...
        For Each ofila In Ods.Tables("detalle_traslado").Rows
            ...
            dr.Item("TipoDoctoOrigen") = Me.cmbTipoDocto.SelectedValue 'ofila.Item("TipoDoctoFactura")
    ...
    Private Sub guardarComentario()
        ...
        Try
            gs_usuario = "CHERNANDEZ"
            lsSQL = "pa_upd_um_documento_comentario4 '" & gs_empresa & "','" & Me.cmbTipoDocto.SelectedValue & "','" & Me.txtNumero.Text & "','" & gs_usuario & "','" & Me.txtComentario4.Text & "'"
    ...
    Private Function generarPDF(psFechaDocto As String) As String
        ...
        Try
            ...
            lsRutaPDF = lsRutaPDF & "\" & Me.cmbTipoDocto.SelectedValue.ToString.Replace(" ", "_") & "_" & Me.txtNumero.Text & ".pdf"
            ...
            pm_valores2(1) = Me.cmbTipoDocto.SelectedValue
            pm_valores2(2) = Me.txtNumero.Text
    ...
    Private Sub enviarCorreo_html()
        ...
        Try
            ...
            sbBody.AppendLine("<td  style='text-align: Left;'>" + gs_empresa + "</td></tr>")


            sbBody.AppendLine("<tr>")
            sbBody.AppendLine("<td><strong>Documento</strong></td>")
            sbBody.AppendLine("<td   style='text-align: Left;'>" + Me.cmbTipoDocto.SelectedValue.ToString & "-" & Me.txtNumero.Text + "</td></tr>")
            ...
            sbBody.AppendLine("<td  style='text-align: Left;'>" + Me.txtFecha.Text + "</td></tr>")
            ...
            Try
                Dim dtBU As DataTable
                Dim dtCorreo As DataTable
                dtDetalle = clsGen.selectQuery("FlexLine", "pa_sel_um_documentod '" & gs_empresa & "','" & Me.cmbTipoDocto.SelectedValue.ToString & "','" & Me.txtNumero.Text & "'")
                ldFechaDocto = dtDetalle.Rows(0).Item("fecha_docto")
                dtBU = clsGen.ValoresDistinto(dtDetalle, "analisisproducto17".Split(","))
                For Each dr As DataRow In dtBU.Rows
                    If (scuentas.ToString().Length > 0) Then scuentas += ","
                    scuentas += "informes_" & dr.Item("analisisproducto17").ToString().Replace(" ", "").Trim.ToLower & "@logiservicios.com"
                Next
            ...
            sbBody.AppendLine("<td>Codigo</td><td>Descripcion</td><td>Cantidad</td></tr>")


            For Each drLinea As DataRow In dtDetalle.Rows
                sbBody.AppendLine("<tr>")

                sbBody.AppendLine("<td>" & drLinea.Item("producto").ToString & "</td>")
                sbBody.AppendLine("<td>" & drLinea.Item("glosa").ToString & "</td>")
                sbBody.AppendLine("<td>" & Format(Convert.ToDecimal(drLinea.Item("cantidad").ToString), "###,###,##0") & "</td>")
                sbBody.AppendLine("</tr>")
            Next
            ...
            'scuentas = "carlos.oscal@umbralcorp.com"
            'clsGen.enviarcorreo(sRemitente, snombreRemitente, scuentas, sSubject, sBody, lsRuta)
            clsGen.enviarcorreo_html(scuentas, sSubject, sbBody.ToString, "", lsRuta, sRemitente, snombreRemitente)
    ...
    Private Sub Limpiar()
        ...
        Me.txtComentario4.Text = String.Empty
        ...
        If MessageBox.Show("Esta Seguro de Aplicar los Cambios", "Confirmacion", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
            Try
                ...
                If validacionesGuardar() Then
                    ...
                    If Me.txtComentario4.Text.Length > 0 Then '(c)20160328 Modificamos el comentario4 del documento Original 
                        guardarComentario()
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        llenarLineasHH(gs_empresa, txtNumero.Text, txtProveedor.Text)
    End Sub


    Private Sub llenarLineasHH(Pempresa As String, PNumero As String, Pproveedor As String)
        Dim clsGen As New ClasesGenerales.General
        Dim dtProductos As DataTable
        Dim lsSQL As String
        Dim lsbodegaIngreso As String = Me.txtBodega.Text
        Dim lsbodegaProducto As String

        Try
            lsSQL = "pa_sel_um_pwa_informe_internacion " & "'" & Pempresa & "','" & Pproveedor & "','" & PNumero & "'"
            dtProductos = clsGen.selectQuery("SCM", lsSQL)

            For Each drProducto As DataRow In dtProductos.Rows

                Dim dr As DataRow
                With drProducto
                    lsbodegaProducto = .Item("bodega")
                    If lsbodegaProducto.Trim.ToLower = lsbodegaIngreso.Trim.ToLower Then
                        Continue For
                    End If
                    dr = Ods.Tables("detalle_traslado").NewRow
                    dr.Item("cantidad") = .Item("cantidadRecibida")
                    dr.Item("bodega") = .Item("bodega")
                    dr.Item("producto") = .Item("codigoproducto")
                    dr.Item("glosa") = .Item("descripcionproducto")
                    dr.Item("lote") = .Item("lote").ToString
                    If .Item("lote").ToString.Trim.Length = 0 Then
                        dr.Item("fechavcto") = System.DBNull.Value
                    Else
                        dr.Item("fechavcto") = .Item("fechavencimiento")
                    End If
                    'dr.Item("fechavcto") = .Item("fechavencimiento")
                    dr.Item("secuenciaOrigen") = .Item("secuenciaorigen")
                    dr.Item("codigomotivo") = .Item("codigomotivo")
                    dr.Item("motivo") = .Item("motivo")
