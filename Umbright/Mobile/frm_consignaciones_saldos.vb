...
Public Class frm_consignaciones_saldos
    ...
    Private Sub Llenar_Listado_Clientes()
        ...
        Dim myOtrans As New Transaccional.Conexion("Corporativo")
        ...
        Try
            ...
            ls_sql = "pa_sel_um_consignacion_aprobado " & gi_cod_empresa_onbase.ToString & ",NULL,NULL"
    ...
    Private Sub Mostrar_Cliente()
        ...
        Try
            ...
            ls_sql = "pa_sel_um_mov_consignacion_aprobado " & gi_cod_empresa_onbase.ToString & ",'" & Me.txt_codigo_cliente_dc.Text & "',NULL"
            dt = myOtrans.Obtiene(ls_sql)
            ...
            Try
                ls_sql = "pa_var_um_mov_consignacion_conteo_encabezado_fechas " & gi_cod_empresa_onbase & ",'" & Me.txt_codigo_cliente_dc.Text & "'"
                dt_fechas = myOtrans.Obtiene(ls_sql)

                ls_sql = "pa_var_um_mov_consignacion_conteo_detalle " & gi_cod_empresa_onbase & ",'" & Me.txt_codigo_cliente_dc.Text & "',NULL"
                dt = myOtrans.Obtiene(ls_sql)
            ...
            ClsGen.Alinear_GridView(Ods.Tables("productos"), Me.dgv_productos, "",
                        ",modificado,", ",codigo,descripcion,saldo,conteo_1,conteo_2,conteo_3,conteo_4,conteo_5,", "",
                        nombre_columnas,
                        "", "", True, True, 300, 0)
    ...
    Private Sub Mostrar_Historial(ByVal _listado As Boolean, ByVal _producto As String, ByVal _cliente As String)
        ...
        Dim dr, dr_aux As DataRow
        Dim ls_sql As String
        Dim lbRenConsignaciones As Boolean = False
        Try
            Otrans.open()
            'nrow = Me.dgv_productos.CurrentCell.RowIndex

            ls_sql = "pa_sel_um_ctacte_ren_consignacion '" & gs_empresa & "','" & _cliente & "'"
            dt = Otrans.Obtiene(ls_sql)
            If dt.Rows.Count > 0 Then
                lbRenConsignaciones = True
            End If


            If lbRenConsignaciones Then

                ls_sql = "pa_sel_um_consignaciones_saldos_re null,'" & gs_empresa & "','" & _cliente & "','" & _producto & "'"
                dt2 = Otrans.Obtiene(ls_sql)

                ls_sql = "pa_sel_um_consignaciones_re null,'" & gs_empresa & "','" & _cliente & "','" &
                        _producto & "'"
            ...
            Else
                ...
                ls_sql = "pa_sel_um_consignaciones null,'" & gs_empresa & "','" & _cliente & "','" &
                        _producto & "'"
            ...
            For Each dr In dt.Rows
                ...
                If dt2.DefaultView.Count > 0 Then
                    ...
                    If dr.Item("fd_tipo").ToString.ToLower.StartsWith("con") Or dr.Item("fd_tipo").ToString.ToLower.Equals("nota de remision") Then
    ...
    Private Sub Guardar_Productos_Aprobados()
        ...
        Try
            ...
            For Each drv In Ods.Tables("productos").DefaultView
                ls_sql = "pa_sel_um_mov_consignacion_aprobado " & gi_cod_empresa_onbase.ToString & ",'" & Me.txt_codigo_cliente_dc.Text & "','" & drv.Item("codigo").ToString & "'"
                dt = myOtrans.Obtiene(ls_sql)

                If dt.Rows.Count > 0 Then
                    ls_sql = "pa_upd_um_mov_consignacion_aprobado  " & gi_cod_empresa_onbase.ToString & ",'" & Me.txt_codigo_cliente_dc.Text & "','" & drv.Item("codigo").ToString & "'," &
                            drv.Item("Aprobado").ToString & ",'" & gs_usuario & "'"
                    myOtrans.Actualiza(ls_sql)
                    myOtrans.Escribir_Log(ls_sql)
                ...
                Else 'debo agregar
                    ls_sql = "pa_ins_um_mov_consignacion_aprobado " & gi_cod_empresa_onbase.ToString & ",'" & Me.txt_codigo_cliente_dc.Text & "','" & drv.Item("codigo").ToString & "'," &
                                                drv.Item("Aprobado").ToString & ",'" & gs_usuario & "'"
