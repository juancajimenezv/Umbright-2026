Public Class frmRequisicionesTrasladoRecepcion
    ...
    Private Sub buscarRequisicion()
        ...
        Try
            ...
            If dt.Rows.Count > 0 Then
                ...
                lsSQL = "pa_sel_um_requisicionImagen '" & Me.cmbEmpresa.SelectedValue & "','" & Me.txtNumeroRequisicion.Text & "'"
                dt = Otrans.Obtiene(lsSQL)
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    If dt.DefaultView.Count > 0 Then
                        ...
                        Me.lblReenvio.Text = ""
                        Me.lblReenvio.Text = "Nombre Imagen Factura " & dt.Rows(0).Item("Nombre").ToString
                        Me.lblReenvio.Visible = True
                    Else
                        MessageBox.Show("No Existe Imagen Asociada", "Verificacion", MessageBoxButtons.OK)
    ...
    Private Sub GuardarEnvio()
        ...
        Try
            ...
            lsSQL = "pa_var_um_requisicion_recepcion_numero"
            dt = Otrans.Obtiene(lsSQL)
            Me.lblNumero.Text = dt.Rows(0).Item("numero")

            lsSQL = "pa_ins_um_requisicionenvio_recepcion '" & Me.lblNumero.Text & "','" & Me.DateTimePicker1.Value & "','" & gs_usuario & "'"
            Otrans.Ingresa(lsSQL)

            If Otrans.Codigo_error = 0 Then
                For Each dr As DataRow In ods.Tables("detalle").Rows
                    lsSQL = "pa_upd_um_requisicionEstado '" & dr.Item("Empresa").ToString & "','" & dr.Item("numero").ToString & "','" & gs_usuario & "',80" & ",'Envio Numero " & Me.lblNumero.Text & "'"
                    Otrans.Actualiza(lsSQL)

                    lsSQL = "pa_upd_um_requisicion_envio_recepcion '" & dr.Item("Empresa").ToString & "','" & dr.Item("numero").ToString & "','" & Me.lblNumero.Text & "'"
                    Otrans.Actualiza(lsSQL)
                Next
    ...
    Private Sub enviarReporte()
        ...
        Try
            sNombreReporte = "envio OC Recepcion"
            ...
            Oaut._reporte_generico(path_reporte, pm_parametros, pm_valores, pm_conexion(0), pm_conexion(1), pm_conexion(2), pm_conexion(3), False, True, "PDF", True)
    ...
    Private Sub llenarEnvios()
        ...
        Try
            Otrans.open()
            lsSQL = "pa_sel_um_requisicionEnvio_Recepcion"
            dt = Otrans.Obtiene(lsSQL)
    ...
    Private Sub mostrarLote(ByVal piLote As Integer)
        ...
        Try
            ...
            limpiarForma()
            dt = Otrans.Obtiene("pa_var_um_requisicion_envio_Recepcion  " & piLote)
